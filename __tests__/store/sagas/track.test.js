...
import { searchSuccess, searchFailure } from '~/store/modules/track/actions';
import { search } from '~/store/modules/track/sagas';
...
describe('Track saga', () => {
  ...
  it('should be able to fetch tracks', async () => {
    ...
    apiMock.onGet(`/artists/${album.value}/albums`).reply(200, response);

    await runSaga({ dispatch }, search, album).toPromise();
  ...
  it('should fail when api returns error', async () => {
    ...
    apiMock.onGet(`/artists/${album.value}/albums`).reply(500);
